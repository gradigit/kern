...
For the canonical contributor-vs-release validation split, see [Kern release validation gate](release-validation-gate.md). Record completed release evidence in the GitHub release notes, release issue, or a fresh versioned evidence file when a maintainer wants that evidence tracked publicly.
